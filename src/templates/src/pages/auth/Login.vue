  ...
  methods: {
    login () {
      ...
      if (!this.$v.data.$error) {
        this.loading = true
        this.$auth.login(this.data.body).then((response) => {
          const token = response.data.token
          this.$store.commit('auth/setUser', response.data.user.data)
          this.$auth.setToken(token, this.data.rememberMe)
          this.$router.push('/account')
        }).catch((error) => {
          if (error.response) {
            if (error.response.status === 401) {
              this.$q.dialog({
                message: this.$t('errors.login.verification_required')
              })
            } else if (error.response.status === 403) {
              this.$q.dialog({
                message: this.$t('errors.login.invalid_credentials')
              })
            } else {
              this.$q.dialog({
